...
export function addDetails({scene,L,box,label,textures,pickables}) {
 ...
 const white=mat('#f6f4ed'),iron=mat('#f1f1ec',{metalness:.35,roughness:.44}),darkMetal=mat('#5c5b5a',{metalness:.65,roughness:.35});
 ...
 const beddingBefore=new Set(scene.children);
 for(const z of [b.z+.75]){
 ...
 const beddingFallback=new THREE.Group();beddingFallback.name='beddingFallback';for(const o of [...scene.children])if(!beddingBefore.has(o))beddingFallback.add(o);scene.add(beddingFallback);
 label('床头 ←',.36,.99,b.z+.65);
 const wardrobeBefore=new Set(scene.children);
 ...
 const wardrobeGroup=new THREE.Group();wardrobeGroup.name='wardrobe';for(const o of [...scene.children])if(!wardrobeBefore.has(o))wardrobeGroup.add(o);scene.add(wardrobeGroup);wardrobeGroup.visible=false;
 ...
 for(let i=0;i<3;i++){
  ...
  B(c.width-.045,hh,.008,cx,y,c.z+c.depth+.008,glass,cab).castShadow=false;
 ...
 const shelfBefore=new Set(scene.children);
 ...
 const shelfGroup=new THREE.Group();shelfGroup.name='shelf';for(const o of [...scene.children])if(!shelfBefore.has(o))shelfGroup.add(o);scene.add(shelfGroup);
 // Acoustic guitar in the gap; all children belong to one transform and sit on a stand.
 const guitar=new THREE.Group();guitar.position.set(L.guitar.x,0,L.guitar.z);guitar.rotation.y=-Math.PI/2;guitar.rotation.x=-.07;scene.add(guitar);
 ...
 label('吉他',L.guitar.x-.06,1.2,L.guitar.z);
 return {wardrobe:wardrobeGroup,shelf:shelfGroup,beddingFallback};
}


export function woodFloorTexture(textures) {
 const c=document.createElement('canvas');c.width=c.height=512;const ctx=c.getContext('2d');
 ctx.fillStyle='#695044';ctx.fillRect(0,0,512,512);
 ...
 for(let i=0;i<340;i++){
  ...
  ctx.strokeStyle=rand()>.5?'rgba(30,23,21,.20)':'rgba(196,164,142,.13)';ctx.lineWidth=.3+rand()*.8;ctx.stroke();
 }
