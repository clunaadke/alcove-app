// Adapted directly from the approved alcove-living-water prototype.
(()=>{
    const root=document.getElementById('alcove-living-water'),screen=root.querySelector('.alw-screen'),canvas=root.querySelector('canvas'),photo=root.querySelector('img');
    const enter=root.querySelector('.alw-enter');
    const postNative=action=>{window.webkit?.messageHandlers?.alcoveSplash?.postMessage(action)};
    ...
    const quotes=window.AlcoveSplashQuotes;
    let textDirty=true;
    function wrapText(text,width){
      const result=[];
      for(const paragraph of text.split('\n')){
        const paragraphStart=result.length;let line='';
        const tokens=/[A-Za-z]/.test(paragraph)?paragraph.match(/\S+\s*|\s+/g)||[]:Array.from(paragraph);
        ...
        // Avoid a lone Chinese character on its own visual line.
    ...
    function paintText(){
      tctx.clearRect(0,0,W,H);tctx.fillStyle='#fff';tctx.textBaseline='top';tctx.textAlign='left';tctx.font='300 11px "Alcove Serif",serif';tctx.shadowColor='rgba(13,48,87,.25)';tctx.shadowBlur=1.4;
      ...
      const blocks=quotes.map((q,i)=>({...settings[i],lines:wrapText(q,settings[i].w)}));
      const lineHeight=17.5,logoHeight=39,total=blocks.reduce((n,b)=>n+b.lines.length*lineHeight,0)+logoHeight;
      const gap=Math.max(3,(bottom-top-total)/7);let y=top;
      // On compact landscape layouts use two columns, preserving all seven passages.
      ...
      if(compact){
        ...
        if(i===2){tctx.textAlign='center';tctx.font='400 35px "Alcove Script",cursive';tctx.fillText('Alcove',W*.51,y);tctx.textAlign='left';tctx.font='300 11px "Alcove Serif",serif';y+=logoHeight+gap}
      });
    ...
    function setup(){preparePhoto();let gl=canvas.getContext('webgl',{alpha:false,antialias:false,powerPreference:'low-power'});if(gl){try{
      ...
      if(!gl){const fallback=make(W,H);fallback.style.cssText='position:absolute;inset:0;width:100%;height:100%;pointer-events:none;z-index:1';fallback.className='alw-canvas';canvas.after(fallback);const ctx=fallback.getContext('2d'),under=make(W,H),u=under.getContext('2d'),comp=make(W,H),cc=comp.getContext('2d');const paintUnder=()=>{u.filter='blur(4px)';drawPhoto(u);u.filter='none';u.fillStyle='rgba(11,56,120,.36)';u.fillRect(0,0,W,H);u.drawImage(textCanvas,0,0,W,H);textDirty=false};paintUnder();
        ...
        // 别把上面 catch 记下的 'fallback' 盖掉 —— 「没有 WebGL」和「WebGL 建到一半抛错」
        // 是两回事，盖掉之后从 dataset 里就再也分不出来了。
    ...
    window.alcoveSplashStop=()=>{stopped=true;cancelAnimationFrame(raf);releaseRenderer()};
    let resizeTimer;
